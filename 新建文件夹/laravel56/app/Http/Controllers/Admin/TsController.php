...
LEFT JOIN train_schools  c ON b.id = c.school_area_id  WHERE b.`city` LIKE '" . $city . "%'   GROUP BY b.`city`,b.id ";
    ...
    public function getTs()
    {
        try{
            ...
            $cityid = $city == '' ? 0 : $city;
            if ($cityid == 0) {
                $where = "SELECT a.id, a.`name`,a.plan_push_nums,a.count_push_nums,a.prev_push_nums,date_format(a.push_time,'%Y-%m-%d') push_time,a.logo,a.school_area_id FROM train_schools a ";
                ...
                $where = "SELECT a.id, a.`name`,a.plan_push_nums,a.count_push_nums,a.prev_push_nums,date_format(a.push_time,'%Y-%m-%d') push_time,a.logo,a.school_area_id  FROM train_schools a where a.school_area_id = " . $cityid;
    ...
    public function upload(Request $request)
    {
        try {
            ...
            $cityid =  request()->route( 'cityid');
                ...
                require_once '../excel/PHPExcel/Classes/PHPExcel/Shared/Date.php';
                ...
                for ($j = 2; $j <= $highestRow; $j++)               //从第二行开始读取数据
                {
                    ...
                    for ($k = 'A'; $k <= $highestColumn; $k++)      //从A列读取数据
                    {
                        if( $k == 'A'){
                            //指定H列为时间所在列
                            $strValue .= "'" . gmdate( "Y/m/d",\PHPExcel_Shared_Date::ExcelToPHP($objPHPExcel->getActiveSheet()->getCell("$k$j")->getValue())) . "'" . ',';
                        }else{
                            //读取单元格
